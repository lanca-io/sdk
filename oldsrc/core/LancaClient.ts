...
export class LancaClient {
  ...
  public async executeRoute(
    ...
    try {
      ...
      const inputRouteData: IInputRouteData = this.buildRouteData(
        route,
        clientAddress,
        destinationAddress
      )
  ...
  public async getSupportedChains(): Promise<ILancaChain[] | undefined> {
    try {
      const supportedChainsResponse: { data: ILancaChain[] } =
  ...
  public async getSupportedTokens({
    ...
    try {
      const supportedTokensResponse: { data: ILancaToken[] } =
        await httpClient.get(conceroApi.tokens, options)
  ...
  public async getRouteStatus(txHash: string): Promise<ITxStep[] | undefined> {
    ...
    const routeStatusResponse: { data: ITxStep[] } = await httpClient.get(
  ...
  private async handleSwitchChain(
    ...
    try {
      ...
      try {
        ...
        throw error instanceof LancaClientError
          ? error
          : globalErrorHandler.parse(error)
      }
    } catch (error) {
      const parsedError =
        error instanceof LancaClientError
          ? error
          : globalErrorHandler.parse(error)
   ...
   */
  private async addChainToWallet(
    walletClient: WalletClient,
    chainId: number
  ): Promise<void> {
    try {
  ...
  private async handleAllowance(
    ...
    const isSwitchStepPresent =
      routeStatus.steps[0].type === StepType.SWITCH_CHAIN
    ...
    const conceroAddress = this.config.testnet
      ? conceroV2AddressesMap[chain.id]
      : conceroAddressesMap[chain.id]
    ...
    try {
      ...
      if (approveTxHash) {
        ...
        if (allowanceVerified) {
          execution!.status = Status.SUCCESS
          ;(execution! as ITxStepSwap).txHash =
            approveTxHash.toLowerCase() as Hash
        } else {
          execution!.status = Status.FAILED
  ...
  private async estimateGas(
    ...
    try {
      ...
      const gasLimit = isOPStack
        ? await (
            publicClient.extend(publicActionsL2()) as PublicClient &
              PublicActionsL2
          ).estimateTotalGas({
            data,
            account: account!,
  ...
  private async handleTransactionStatus(
    ...
    const isBridgeStepExist = routeStatus.steps.some(
      ({ type }) => type === StepType.BRIDGE
    )

    if (
      status === 'success' &&
      firstStepType?.type === StepType.SRC_SWAP &&
      !isBridgeStepExist
    ) {
      let step
      do {
        ...
        if (!step) {
          await new Promise((resolve) =>
            setTimeout(resolve, DEFAULT_REQUEST_TIMEOUT_MS)
          )
        }
      } while (!step)
  ...
  private async fetchRouteSteps(txHash: Hash): Promise<ITxStep[]> {
    ...
    const { data: steps }: { data: ITxStep[] } = await httpClient.get(
      conceroApi.routeStatus,
      options
    )
    return steps
   ...
   * transaction failed.
   */
  private evaluateStepsStatus(steps: ITxStep[]): {
    status: Status
    newTxHash?: Hash
    error?: string
  } {
    ...
    const anyFailed = steps.some(
      ({ status }: { status: Status }) => status === Status.FAILED
    )
  ...
  private updateRouteSteps(
    ...
    routeStatus.steps.forEach((step) => {
      const isNewStep =
        step.type !== StepType.SWITCH_CHAIN && step.type !== StepType.ALLOWANCE
      if (isNewStep) {
        step.execution = {
          ...step.execution,
          ...txSteps[indexOfStep++],
  ...
  private setAllStepsData(
    ...
    routeStatus.steps.forEach((step) => {
      if (
        step.type !== StepType.SWITCH_CHAIN &&
        step.type !== StepType.ALLOWANCE
      ) {
  ...
  private buildRouteData(
    ...
    steps.forEach((step) => {
      ...
      if (type === StepType.BRIDGE) {
        ...
        ;(step as IRouteStep).internalSteps.forEach(
          (internalStep: IRouteInternalStep) => {
